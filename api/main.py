...
async def get_fotos_publicas(album_id: int):
    try:
        ...
        fotos = os.listdir(f"imagens/{album.nome}")
...
async def get_foto_publica(album_id: int, foto: str, redis_cache: cache = Depends(cache)):
    try:
        ...
        image_path = f"imagens/{album['nome']}/{foto}.webp"
        if not os.path.exists(image_path):
            original_image_path = f"imagens/{album['nome']}/{foto}.jpg"
...
async def get_foto_full_quality(album_id: int, foto: str, redis_cache: cache = Depends(cache)):
    try:
        cache_key = f"album_{album_id}"
        cached_profile = redis_cache.hmget(cache_key, "id", "nome", "publico")
        ...
        image_path = f"imagens/{album['nome']}/{foto}.jpg"
        if not os.path.exists(image_path):
            return Response(content="Foto não encontrada", status_code=404)
...
async def create_foto(album_id: int, foto: str, foto_file: Annotated[bytes, File()], redis_cache: cache = Depends(cache)):
    try:
        cache_key = f"album_{album_id}"
        cached_profile = redis_cache.hmget(cache_key, "id", "nome", "publico")
        ...
        with open(f"imagens/{album['nome']}/{foto}.jpg", "wb") as f:
            f.write(foto_file)
...
async def delete_foto(album_id: int, foto: str, redis_cache: cache = Depends(cache)):
    try:
        ...
        image_path = f"imagens/{album['nome']}/{foto}.jpg"
        image_path_webp = f"imagens/{album['nome']}/{foto}.webp"
